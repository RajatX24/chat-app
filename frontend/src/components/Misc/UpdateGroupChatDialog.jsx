...
function SimpleDialog(props) {
  ...
  const addUser = async (user1) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_SERVER}/api/chat/groupadd`,
        {
  ...
  const handleRemove = async (user1) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_SERVER}/api/chat/groupremove`,
        {
          chatId: selectedChat._id,
          userId: user1._id,
        },
  ...
  const handleRename = async () => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_SERVER}/api/chat/rename`,
        {
          chatId: selectedChat._id,
          chatName: grpChatNm,
        },
  ...
  const handleSearch = async (search) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_SERVER}/api/user/allusers?search=${search}`,
        config
      );
      setLoading(false);
